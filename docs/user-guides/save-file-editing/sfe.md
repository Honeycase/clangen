...
Want to edit the development versions save files? Check out [development's tab](development.md). 
...
When editing ClanGen's saves, you're going to run into `JSON` files. `JSON` is a lightweight, text-based data format that is readable by humans and easily generated by machines. 

`JSON` is CASE SENSITIVE. It is important to keep in mind that `JSON` is still a code file, thus expect miniscule errors (such as missing commas & brackets) to be flagged and prevent you from running the save.
...
- Check your cats sprite information. If they have a number in quotes rather than something like to "newborn0", then you'll have to change them to reflect the new sprite data options for 0.13.
...
- nutrition_info.json has an ID present in the file that is either dead or not present in the "clan_cats" list within the saves `clan.json`. To fix, simply remove the ID and the connected information from the nutrition file
